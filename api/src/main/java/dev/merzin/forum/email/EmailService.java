...
public class EmailService {
  ...
  public void sendVerificationEmail(String username) throws Exception {
    ...
    var linkedAccount =
        accountRepository.findByVerifiedEmail(account.getEmail());
    if (linkedAccount.isPresent())
      throw new Exception("Email already in use");
    var verification = emailVerificationRepository
                           .findPendingByAccountUsernameAndEmailAndCreatedAfter(
    ...
    if (verification == null) {
      ...
      if (verification.getSent().toInstant().isAfter(after))
        throw new Exception("Email already sent");
    }
    var email = Email.builder()
                    .recipient(account.getEmail())
                    .subject("Verify your email")
                    .body(formatVerificationEmail(account.getUsername(),
                                                  verification.getId()))
                    .build();
    sendEmail(email);
  ...
  private String formatVerificationEmail(String username, UUID verificationId)
      throws IOException {
    var template = StreamUtils.copyToString(
        verificationEmailTemplate.getInputStream(), StandardCharsets.UTF_8);
    return template.replaceAll("%USERNAME%", username)
        .replaceAll("%ID%", verificationId.toString());
  }
